...
def get_ups_downs_statistic(topic):
    ...
    plt.figure(figsize=(15, 8))
    fig, ax = plt.subplots()
    ax.pie([ups[5], downs[5]])
    ax.plot([], [], color='blue', label='up votes', linewidth=2)
    ax.plot([], [], color='orange', label='down votes', linewidth=2)
    # ax.pie([ups[5], downs[5]])
    ax.set(title='Ups-Downs in the last 10 minutes in {topic}'.format(topic=topic))
    ax.grid()
    ax.legend()
